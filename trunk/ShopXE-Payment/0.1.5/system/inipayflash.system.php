...
class inipayflash extends PaymentSystem
{
	
	function inipayflash($payment_info, $args) { return parent::PaymentSystem($payment_info, $args); }
	...
	function getJsFile()
	{
		return Array("https://plugin.inicis.com/pay40_host.js", "https://plugin.inicis.com/inipayFlash/pay60_flash.js");
	}

	function getCallScript()
	{

		Context::addHtmlFooter("<div id=\"install_layer_xpay\" name=\"install_layer_xpay\" style=\"position:absolute;top:0px;left:0px;\"></div>");
		return "
			jQuery('body').css('overflow', 'hidden');
			var top = jQuery(document).scrollTop();

			jQuery('#install_layer_xpay').css('top', top).width('100%').height('100%');		
			inipayFlash_load( 'install_layer_xpay', '3', '100%', '100%', '".$this->args->result_page."', 'false');
			";
	}
	...
	StartSmartUpdate();
		...
		function F_Close(){
			...
			jQuery('#install_layer_xpay').html('').width(0).height(0);
